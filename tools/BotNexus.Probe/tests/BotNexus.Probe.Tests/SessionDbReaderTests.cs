...
public sealed class SessionDbReaderTests
{
    ...
    public async Task SessionDbReader_ReadsSessionMetadataAndHistory()
    {
        ...
        sessions[0].Id.ShouldBe("s-1");
        sessions[0].MessageCount.ShouldBe(2);
        ...
        history[1].ToolName.ShouldBe("search");
    ...
    public async Task SessionDbReader_ReturnsStatusCounts()
    {
        ...
        counts.Active.ShouldBe(1);
        counts.Sealed.ShouldBe(1);
        counts.Expired.ShouldBe(0);
        counts.Suspended.ShouldBe(0);
    }
