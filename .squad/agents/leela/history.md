...
## Learnings — Cross-Platform Imaging Fix (2025-01-03)

1. **Defensive optimization becomes technical debt when platforms diverge** — The image resizing logic in `ReadTool.cs` used `System.Drawing.Common`, which is Windows-only on .NET 6+. The catch-block fallback meant Linux/macOS already skipped resize silently. Making this behavior explicit across all platforms (by removing resize entirely) eliminated the platform-specific code and the CA1416 warning.
2. **Modern AI models make client-side image optimization unnecessary** — GPT-4o, Claude 3.5, and other multimodal models handle large images efficiently. The 2000px resize threshold was conservative. Removing client-side resize simplifies the codebase without practical loss. If optimization becomes necessary later, it's better implemented server-side or with a proper cross-platform library (ImageSharp, SkiaSharp).
3. **Simplicity is an architectural choice** — When faced with cross-platform API issues, consider whether the feature is truly necessary. In this case, removing image resizing entirely was architecturally superior to adding new dependencies (SkiaSharp, ImageSharp) or runtime guards (`OperatingSystem.IsWindows()`). The simplest solution that meets requirements is often the right one.

## Learnings — OTel + Serilog Observability Proposal (2026-04-08)

1. **Logging is sparse but well-structured** — Only 73 `_logger.*` call sites across 20 files, but all use `Microsoft.Extensions.Logging` correctly. Only 4 projects have explicit M.E.L.Abstractions references; the rest rely on framework transitive dependencies. This means Serilog adoption is a host-level swap with zero business code migration.
2. **CorrelationIdMiddleware is the OTel bridge point** — The existing `X-Correlation-Id` middleware in Gateway.Api already generates and propagates correlation IDs. This is the natural integration point for `Activity.Current.TraceId` bridging. Don't replace it — evolve it to work with both systems.
3. **Four `ActivitySource` instances, not one** — Splitting by architectural layer (Gateway, Providers, Channels, Agents) keeps spans organized and allows selective sampling. Each layer's core library owns its `ActivitySource` definition; the host project registers all of them.
